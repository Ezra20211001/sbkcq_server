...
bool XmlDocument::LoadFile(file_system* p_vfs, LPCTSTR szFilename, XmlEncoding encoding )
{
	...
	DWORD dw_size = m_pVFS->get_size(szFilename);
	if( !VALID_VALUE(dw_size) )
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		return false;
	}
	...
	*/
	...
	char* buf = new char[ dw_size+1 ];
	buf[0] = 0;

	
	if( m_pVFS->load_file(buf, szFilename) != dw_size ) 
	{
		delete[](buf);
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		return false;
	...
	// PRINT_MESSAGE(_T("tinyxml time=%d:%s\r\n"), timeGetTime()-dw_time, szFilename);
